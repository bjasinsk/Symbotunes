...
class Encoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_out = nn.Linear(
            in_features=hidden_size * 2, out_features=z_size * 2
        )

    def reparametrisation_trick(
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        _, (out, _) = self.lstm(
            x, (h0, c0)
        )  # we want the last state from both directions
        out = out.permute(
            1, 0, 2
        ).reshape(
...
class Decoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.linear_in = nn.Linear(
            in_features=z_size, out_features=conductor_hidden * conductor_num_layers * 2
        )
    ...
    def forward(self, z: torch.Tensor, x: torch.Tensor | None = None) -> torch.Tensor:
        ...
        hidden_conductor = [
            hidden_conductor[0].contiguous(),
            hidden_conductor[1].contiguous(),
        ]
        ...
        notes = torch.zeros(
            batch_size, self.total_notes, self.num_tokens, device=z.device
        )
        for subsequence in range(self.num_subsequences):
            conductor_out, hidden_conductor = self.conductor(
                conductor_in, hidden_conductor
            )
            conductor_out = conductor_out.squeeze(1)
            decoder_in = self.linear_pre_decoder(conductor_out)
            decoder_state = decoder_in.view(
                decoder_in.shape[0], self.lstm_num_layers, -1
            ).permute(1, 0, 2)
            ...
            if x is None:
                ...
                for i in range(self.n_per_subseq):
                    ...
                    out, (decoder_h, decoder_c) = self.decoder_lstm(
                        lstm_in, (decoder_h, decoder_c)
                    )
                    out = out.squeeze(1)
                    out = self.linear_out(out)
                ...
                sequence_x = x[
                    :,
                    subsequence * self.n_per_subseq + 1 : (subsequence + 1)
                    * self.n_per_subseq,
                ]
                ...
                notes[
                    :,
                    subsequence * self.n_per_subseq : (subsequence + 1)
                    * self.n_per_subseq,
                ] = out
